...
public class mainMenu {
    ...
    private static void play() throws IOException, InterruptedException {
        Scene.clear();
        Print.header("PLAY", "-");
        Scene.choice(new String[]{"START A NEW ADVENTURE", "LOAD GAME", "GO BACK", "QUICK START [DEV OPTION]"});
        Print.text("Response: ");
        int choice = Input.Int();
        if (choice == 1) {
            if (Save.SaveExists()) {
                Print.text("You already have a saved game. Do you still want to start a New Game? Your progress will reset... [Y/N]\n\nResponse: ");
                String response = Input.String();
            ...
            if (Save.SaveExists()) {
                ...
                Print.error("Save file does not exist or has been tampered with");
                Thread.sleep(3000);
                Print.text("Do you want to start a New Game? [Y/N]\n\nResponse: ");
                String response = Input.String();
                if (response.equalsIgnoreCase("y")) {
                    Save.make();
                    LoadGame.main(new String[]{});
    ...
    private static void settings() throws IOException, InterruptedException {
        ...
        if (choice == 1) {
            ...
            Print.header("Game Version: Summer Update | v0.0.10 Development Build");
            Print.header("Check out GitHub page for latest updates!");
            Thread.sleep(5000);
            settings();
        } else if (choice == 3) {
            Scene.clear();
